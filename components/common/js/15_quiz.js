...
// Добавлены роли: Развиваю сотрудников, Найти себя / определиться с путём
// Версия: подсветка выбора + затемнение остальных карточек
// Исправлен сброс подсветки при перезапуске квиза
// ============================================================
(function () {
    let quizQuestions = [];
    ...
    function startQuiz() {
        ...
        updateCardDimming(false);  // <--- СБРОС ПОДСВЕТКИ ПРИ ПЕРЕЗАПУСКЕ
        renderQuiz();
    }
    ...
    function updateCardDimming(hasSelection) {
        const grid = document.querySelector('.quiz-results-grid');
        if (grid) {
            if (hasSelection) {
    ...
    function renderResult(recommendations) {
        ...
        document.querySelectorAll('.choose-option').forEach(btn => {
            ...
            btn._choiceHandler = (e) => {
                ...
                // Подсветка выбранной карточки и затемнение остальных
