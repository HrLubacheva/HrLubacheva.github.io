// ============================================================
// 15_quiz.js – Квиз: вопросы, рендер, сохранение выбора
// ============================================================
(function () {
    let quizQuestions = [];
    ...
    function renderResult(topTwo) {
        ...
        const categoryA = esc(displayNameA.split(' — ')[0] || '');
        const nameA = esc(displayNameA.split(' — ')[1] || displayNameA);
        const categoryB = esc(displayNameB.split(' — ')[0] || '');
        ...
        let html = `<p style="font-weight:600; margin-bottom:20px;">На основе ваших ответов мы подобрали оптимальные варианты:</p><div style="display:flex; flex-wrap:wrap; gap:20px; justify-content:center;"><div style="flex:1; min-width:250px; background:white; border-radius:28px; padding:28px; border:1px solid #e0e0e0;"><div style="font-size:32px; margin-bottom:12px;">📌</div><h4>Рекомендация 1</h4><p style="font-size:0.85rem; color:#666; margin-bottom:8px;">${categoryA}</p><p><strong>${nameA}</strong></p><p class="service-price" style="color: var(--primary); font-weight: 700; margin: 10px 0;">💰 ${priceAEsc}</p><button class="btn-primary choose-option" data-choice="1" data-text="${varA}" data-price="${priceAEsc}" data-display="${displayA}">Узнать подробнее →</button></div><div style="flex:1; min-width:250px; background:white; border-radius:28px; padding:28px; border:1px solid #e0e0e0;"><div style="font-size:32px; margin-bottom:12px;">🎯</div><h4>Рекомендация 2</h4><p style="font-size:0.85rem; color:#666; margin-bottom:8px;">${categoryB}</p><p><strong>${nameB}</strong></p><p class="service-price" style="color: var(--primary); font-weight: 700; margin: 10px 0;">💰 ${priceBEsc}</p><button class="btn-primary choose-option" data-choice="2" data-text="${varB}" data-price="${priceBEsc}" data-display="${displayB}">Узнать подробнее →</button></div><div style="flex:1; min-width:250px; background:#f8f9fa; border-radius:28px; padding:28px; border:1px solid #e0e0e0;"><div style="font-size:32px; margin-bottom:12px;">🤔</div><h4>Вариант 3</h4><p><strong>Не уверены в выборе?</strong></p><p style="font-size:0.9rem; margin-bottom:10px;">Позвольте помочь – бесплатная консультация 15 минут.</p><button class="btn-secondary choose-help" data-choice="help" data-text="Помогите выбрать (бесплатная консультация)" data-price="">Помогите выбрать →</button></div></div><div style="text-align:center; margin-top:20px;"><button id="resetQuizBtn" class="btn-secondary">🔁 Пройти заново</button></div>`;
        container.innerHTML = html;
        document.querySelectorAll('.choose-option').forEach(btn => {
            ...
            btn._choiceHandler = () => {
                ...
                const answersStr = answers.map((a, idx) => { const qText = ['Роль', 'Уровень', 'Срочность', 'Важность', 'Бюджет'][idx]; return `${qText}: ${a || 'не выбран'}`; }).join('\n');
        ...
        if (helpBtn) {
            ...
            helpBtn._helpHandler = () => {
                ...
                window.showSuccessToast('Спасибо!');
                showFinalScreen();
                isSubmittingChoice = false;
            };
        ...
        function showFinalScreen() {
            container.innerHTML = `<p style="text-align:center; font-size:1.2rem; margin-bottom:20px;">✨ Спасибо за прохождение квиза! ✨</p><p style="text-align:center; margin-bottom:20px;">📋 Заполните форму ниже, и я свяжусь с вами.</p><div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 20px;"><button id="goToCallbackBtn" class="btn-primary" style="min-width: 200px;">📝 Перейти к форме заявки</button><button id="goToCalculatorBtn" class="btn-secondary" style="min-width: 200px;">🔍 Посмотреть все услуги</button></div><div style="text-align:center; margin-top:20px;"><button id="resetQuizBtnAfter" class="btn-secondary">🔁 Пройти заново</button></div>`;
            const goToCallback = document.getElementById('goToCallbackBtn');
            const goToCalculator = document.getElementById('goToCalculatorBtn');
            if (goToCallback) goToCallback.addEventListener('click', () => { const target = document.getElementById('callback-form'); if (target && window.smoothScrollTo) window.smoothScrollTo(target); });
            if (goToCalculator) goToCalculator.addEventListener('click', () => { const target = document.getElementById('calculator'); if (target && window.smoothScrollTo) window.smoothScrollTo(target); });
    ...
    window.initQuiz = function () {
        ...
        if (removeBtn) {
            ...
            removeBtn._removeHandler = function () { selectedVariantText = ''; selectedVariantPrice = ''; updateFormHiddenFields('', '', selectedOriginalText, selectedOriginalPrice, '', ''); updateSelectionBlock('', ''); window.showSuccessToast('Выбранный вариант удалён'); };
