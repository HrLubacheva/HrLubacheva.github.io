// ============================================================
// 16_email.js – Отправка материалов (чек-лист, программа) на email
// ============================================================
const MAX_RETRIES = 3;
...
async function sendMaterialsEmail(email, wantChecklist, wantTraining) {
    ...
    try { const result = await window.postWithRetry(url, formData, MAX_RETRIES, RETRY_DELAY); lastSendTime = Date.now(); return result; } finally { setTimeout(() => { isSendingMaterials = false; }, 1000); }
}
...
async function sendMaterialsToEmail(email, materialType) {
    ...
    isSendingEmail = true;
    try { await sendMaterialsEmail(email, wantChecklist, wantTraining); window.showSuccessToast('✅ Материалы отправлены! Проверьте почту'); return true; } catch (err) { window.showErrorToast('❌ Ошибка: ' + err.message); return false; } finally { setTimeout(() => { isSendingEmail = false; }, 1000); }
}
