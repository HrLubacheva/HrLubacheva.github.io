...
window.postWithRetry = async function (url, data, retries = null, baseDelay = null) {
    // Параметры retries и baseDelay не используются, но оставлены для совместимости
    try {
        const body = new URLSearchParams(data);
        // Отправляем запрос в режиме no-cors — браузер не ждёт ответа и не блокирует CORS
        await fetch(url, {
            method: 'POST',
            body: body,
            mode: 'no-cors',
        ...
        // Логируем ошибку только в консоль (не влияет на пользователя)
        console.log('Отправка (игнорируем):', err);
    }
    // Всегда возвращаем успех — пользователь увидит зелёный тост
    return true;
};
